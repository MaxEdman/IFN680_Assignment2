...
def siamese_network(x_train, y_train, x_test, y_test):
    ...
    # reshape the input arrays to 4D (batch_size, rows, columns, channels)
    img_rows, img_cols = x_train.shape[1:3]
    pairs_train = x_train.reshape(x_train.shape[0], img_rows, img_cols, 1)
    pairs_test = x_test.reshape(x_test.shape[0], img_rows, img_cols, 1)
    ...
    indices = [np.where(x_train == i)[0] for i in range(num_classes)]
    #input_train_pairs, train_labels = create_pairs(x_train, indices)
    ...
    print("input_train_pairs: ", input_train_pairs.shape)
    print("train_labels: ", train_labels.shape)
    print("input_test_pairs: ", input_test_pairs.shape)
    ...
    model.fit([input_train_pairs[:,0], input_train_pairs[:,1]], 
              train_labels,
